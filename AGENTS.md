...
- 已完成 `显示分数和最高分，并复刻字体类型`。
- `GameWidget` 已新增 `score_` 和 `highScore_`，运行中按帧累积分数，碰撞失败后停止加分。
- `resetGame()` 已支持重开时清空当前分数，同时保留最高分。
- 已通过 `QFontDatabase` 加载 `Resources/TEXTS.ttf`，并使用加载到的字体绘制分数、最高分和失败提示，加载失败时回退到 `Consolas`。
- 分数显示格式已对齐 `QtDino`：右上角显示当前分数和 `HI` 最高分，使用 5 位数字补零。
- 本次提交信息：`显示分数和最高分，并复刻字体类型`。

### 2026-05-05 17:01

- 已完成并推送 `实现下蹲/冲刺和空中快速下落，冲刺额外加分`。
- `GameWidget` 已加载 `Sprint1.png` 和 `Sprint2.png`，地面下蹲/冲刺状态下显示两帧冲刺动画。
- 已新增 `sprint_`、`downPressed_` 和 `spacePressed_`，通过持续按键状态处理下蹲、快速下落和下蹲期间长按跳跃的衔接。
- 空中按下蹲键会将 `velocityY_` 设置为 `FallVelocity = -1840`，落地后可继续保持下蹲/冲刺状态。
- 下蹲状态下碰撞框切换为低姿态矩形，普通状态仍使用原有站立碰撞框。
- `keyPressEvent()` 和 `keyReleaseEvent()` 已过滤自动重复按键事件，减少长按下蹲时闪现普通跑步图的问题。
- 冲刺状态下每帧额外加分，复刻原版冲刺加分效果。
- 对应提交：`e78b5f3`，提交信息：`实现下蹲/冲刺和空中快速下落，冲刺额外加分`。

### 2026-05-05 17:46

- 已完成并验收 `实现飞鸟障碍物`。
- `GameWidget` 已加载 `Bird1.png` 和 `Bird2.png`，用于绘制飞鸟两帧动画。
- 已新增 `ObstacleType`，当前支持区分仙人掌和飞鸟障碍物。
- 障碍物离开左侧后会切换为飞鸟，飞鸟使用 `QRect(width(), 450, 200, 150)` 作为绘制区域。
- 飞鸟动画帧切换已放在 `tick()` 中更新，`paintEvent()` 只根据 `currentBirdFrame_` 负责绘制当前帧。
- 飞鸟碰撞框已缩小为 `QRect(x + 40, y + 45, 120, 60)`，用于验证下蹲/冲刺碰撞框可以躲避低飞飞鸟。
- 本次提交信息：`实现飞鸟障碍物`。

### 2026-05-05 18:22

- 已完成并验收 `实现分数驱动的速度提升`。
- `GameWidget` 已新增 `speed_ = 2`，作为当前游戏速度档位。
- `tick()` 已根据 `score_ >= 3000` 和 `score_ >= 6000` 将速度档位提升到 3 和 4。
- `scrollSpeed_` 已改为由 `speed_ * 3` 派生，`obstacleSpeed_` 已改为由 `speed_ * 8` 派生，保持初始视觉速度不变。
- 分数增长已从固定每帧加 1 改为每帧增加 `speed_ - 1`，冲刺状态额外再增加 `speed_ - 1`。
- `resetGame()` 已支持重开时将速度恢复到初始档位。
- 本次提交信息：`实现分数驱动的速度提升`。

### 2026-05-05 19:00

- 已完成并验收 `实现分层背景动画`。
- `GameWidget` 已加载 `LAYER_STAR.png`、`LAYER_MOON.png`、`LAYER_CLOUD.png` 和 `LAYER_GROUND.png` 四层背景资源。
- 已新增四个背景绘制矩形，统一使用 `QRect(0, 0, 3200, 800)` 对齐当前窗口尺寸和 `QtDino` 背景方案。
- `tick()` 已按 `speed_` 的不同倍率移动星星、月亮、云和地面背景，形成视差滚动效果。
- 背景层移动到 `x <= -1600` 后会回到 `0`，避免循环滚动时出现空白。
- `paintEvent()` 已改为先绘制真实分层背景，再绘制障碍物、恐龙、分数和失败提示。
...
- 已完成并验收 `实现飞鸟随机高度`。
- `spawnNextObstacle()` 中的飞鸟分支已从固定 `y = 450` 改为 `QRandomGenerator::global()->bounded(200, 450)`。
- 飞鸟绘制区域仍保持 `QRect(width(), birdY, 200, 150)`，尺寸不变，只随机高度。
- 飞鸟碰撞框继续基于 `obstacleRect_.y()` 动态计算，因此会自动跟随随机高度。
- 本阶段仍保持单个活动障碍物循环，未引入原版的计时器、概率和多槽位生成机制。
- 本次提交信息：`实现飞鸟随机高度`。

### 2026-05-05 20:30

- 已完成并验收 `随机生成首个障碍物`。
- `GameWidget` 构造函数在资源和字体加载完成后调用 `spawnNextObstacle()`，避免程序启动后的第一个障碍物固定为默认小仙人掌。
...
- 已完成并验收 `调整生命值 UI 位置`。
- 生命值 UI 已从左上角 `QRect(20, 20, 150, 50)` 调整为左下角 `QRect(20, 700, 200, 60)`。
- 位置和尺寸已对齐 `QtDino` 中的 `HeartRect`，为后续体力 UI 预留右侧区域。
- 本次提交信息：`调整生命值 UI 位置`。

### 2026-05-05 21:54

- 已完成并验收 `实现欢迎界面和暂停`。
- `GameWidget` 已新增 `welcome_` 和 `paused_`，并保留 `gameOver_` 作为失败状态。
- `tick()` 已在暂停、欢迎和失败状态下只刷新画面，不继续更新运动、碰撞和分数。
- 欢迎界面会显示 `Welcome to Run's Dino!` 和开始提示，按 `Space`、`Up` 或 `W` 后开始游戏并保留开始即跳效果。
- 游戏中按 `Esc` 可暂停/继续，暂停时显示 `Paused` 和继续提示；欢迎和失败状态下不会触发暂停。
- 失败后按 `R` 会重置局面并直接开始新一局，贴近 `QtDino` 的 `resetRound(false)` 行为。
- 本次提交信息：`实现欢迎界面和暂停`。

### 2026-05-05 22:08

- 已完成并验收 `实现体力显示`。
- `GameWidget` 已新增 `stamina_ = 3`，作为当前体力值状态。
- 已加载 `Stamia0.png`、`Stamia1.png`、`Stamia2.png` 和 `Stamia3.png` 四张体力 UI 资源。
- `paintEvent()` 已根据当前体力值选择对应贴图，并在生命值右侧 `QRect(270, 700, 200, 60)` 绘制体力 UI。
- `resetGame()` 已支持重开时恢复 3 点体力。
- 本阶段只显示体力，不消耗体力，为后续火球攻击、能量球恢复和无敌状态做前置准备。
- 本次提交信息：`实现体力显示`。

### 2026-05-05 22:33

- 已完成并验收 `实现火球攻击`。
- `GameWidget` 已加载 `Fireball.png`，并新增单个火球状态 `fireballActive_` 和 `fireballRect_`。
- 游戏中按 `Right` 可在体力大于 0 且当前没有火球时发射火球，并消耗 1 点体力。
- 火球从原版对应的嘴部位置发射：`x = 220`，`y = 恐龙碰撞框 y + 20`，尺寸为 `50 x 50`。
- `tick()` 已支持火球每帧向右移动 30 像素，超过右侧远端后自动失效。
- 火球命中当前障碍物碰撞框后会失效，并调用 `spawnNextObstacle()` 刷新下一个随机障碍物。
- `resetGame()` 已支持重开时清空火球状态并恢复体力。
- 当前阶段仍保持单个活动火球和单个活动障碍物循环，暂不引入火球数组或多障碍物槽位。
- 本次提交信息：`实现火球攻击`。

### 2026-05-05 22:43

- 已完成并验收 `修复欢迎界面可以发射火球`。
- `keyPressEvent()` 已在欢迎界面单独处理开始输入，只允许 `Space`、`Up` 和 `W` 进入游戏。
- 欢迎界面下按 `Right` 不再发射火球或消耗体力。
- 欢迎界面下按 `Down` 或 `S` 不再提前留下下蹲/冲刺输入状态。
- 跳跃按键分支中原有的欢迎界面开始逻辑已移除，避免状态处理分散。
- 本次提交信息：`修复欢迎界面可以发射火球`。

### 2026-05-05 23:03

- 已完成并验收 `实现能量球恢复体力`。
- `ObstacleType` 已新增 `Energy`，随机障碍物生成范围扩展到 8 种类型。
- `GameWidget` 已加载 `Energyball.png`，并在障碍物绘制分支中支持能量球贴图。
- 能量球生成时使用 `QRect(width(), energyY, 50, 50)`，高度随机范围为 `150 <= y < 450`。
- 能量球碰撞框缩小为中心 `30 x 30`，即 `QRect(x + 10, y + 10, 30, 30)`。
- 恐龙碰到能量球时会恢复 1 点体力，上限为 3，不扣生命，并立即刷新下一个随机障碍物。
- 本次提交信息：`实现能量球恢复体力`。

### 2026-05-05 23:18

- 已完成并验收 `实现无敌状态`。
- `GameWidget` 已新增 `godMode_` 和 `godModeFrames_`，用于记录无敌状态和剩余持续帧数。
- 已新增 `GodModeFrames = 200`，无敌状态会在游戏运行中按帧倒计时并自动结束。
- 游戏中按 `Left` 时，如果体力为 3 且当前不在无敌状态，会消耗全部体力并进入无敌状态。
- 无敌状态下碰到仙人掌或飞鸟不会扣生命，能量球仍可正常恢复体力并刷新障碍物。
- 绘制恐龙时已在无敌状态下使用半透明效果，和普通状态区分。
...
- 进行 `MyDino/` 后续复刻讲解、阶段判断或计划调整时，可以参考计划文件 `C:\Users\ROG\Nutstore\1\我的坚果云\C++大作业\Qt项目复现Todo.md`，其中记录了当前已完成阶段和后续依赖顺序。
